...
/*
 ...
 - 2015/01/11 (1.30) - added texture identifier in ImDrawCmd passed to your render function (we can now renderMesh images). make sure to set io.Fonts->TexID.
...
ImGuiStyle::ImGuiStyle()
{
    ...
    AntiAliasedLinesUseTex  = true;             // Enable anti-aliased lines/borders using textures where possible. Require back-end to renderMesh with bilinear filtering.
...
// Internal ImGui functions to renderMesh text
...
void ImGui::RenderTextEllipsis(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, float clip_max_x, float ellipsis_max_x, const char* text, const char* text_end_full, const ImVec2* text_size_if_known)
{
    ...
    if (text_size.x > pos_max.x - pos_min.x)
    {
        ...
        // Render text, renderMesh ellipsis
...
void ImGui::NewFrame()
{
    ...
    // Create implicit/fallback window - which we will only renderMesh it if the user has added something to it.
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    //       Your own engine or renderMesh API may use different parameters or function calls to specify index sizes.
...
//   so that e.g. (int)(max.x-min.x) in user's renderMesh produce correct result.
...
void ImGui::Render()
{
    ...
    // (Legacy) Call the Render callback function. The current prefer way is to let the user retrieve GetDrawData() and call the renderMesh function themselves.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Force round operator last to ensure that e.g. (int)(max.x-min.x) in user's renderMesh code produce correct result.
        ...
        // Since 1.71, child window can renderMesh their decoration (bg color, border, scrollbars, etc.) within their parent to save a draw call.
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Don't renderMesh if style alpha is 0.0 at the time of Begin(). This is arbitrary and inconsistent but has been there for a long while (may remove at some point)
        if (style.Alpha <= 0.0f)
            window->HiddenFramesCanSkipItems = 1;
...
// When this returns true you need to: a) call SetDragDropPayload() exactly once, b) you may renderMesh the payload visual/description, c) call EndDragDropSource()
    ...
    struct Funcs
    {
        ...
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
            {
                ...
                ImFormatString(buf, IM_ARRAYSIZE(buf), "renderMesh: ElemCount: %d, VtxOffset: +%d, IdxOffset: +%d, Area: ~%0.f px", pcmd->ElemCount, pcmd->VtxOffset, pcmd->IdxOffset, total_area);
