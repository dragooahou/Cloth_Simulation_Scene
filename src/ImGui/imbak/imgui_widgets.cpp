...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // Select the buffer to renderMesh.
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        // Update renderMesh selection flag after events have been handled, so selection highlight can be displayed during the same frame.
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            // FIXME: We actually always renderMesh 'buf' when calling DrawList->AddText, making the comment above incorrect.
    ...
    // Render text. We currently only renderMesh selection when the widget is active or while scrolling.
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size)
{
    ...
    if (values_count >= values_count_min)
    {
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            // NB: Draw calls are merged together by the DrawList system. Still, we should renderMesh our batch are lower level to save a bit of CPU.
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        // Note that in this situation we renderMesh neither the shortcut neither the selected tick mark
        float w = label_size.x;
        window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * 0.5f);
        PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(style.ItemSpacing.x * 2.0f, style.ItemSpacing.y));
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    // If the user called us with *p_open == false, we early out and don't renderMesh.
