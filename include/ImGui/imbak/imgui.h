...
struct ImDrawData;                  // All draw command lists required to renderMesh the frame + pos/size coordinates to use for the projection matrix.
...
namespace ImGui
{
    ...
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by Render(). If you don't need to render data (skipping rendering) you may call EndFrame() without Render()... but you'll have wasted CPU already! If you don't need to renderMesh, better to not create any windows and not call NewFrame() at all!
    IMGUI_API void          Render();                                   // ends the Dear ImGui frame, finalize the draw data. You can get call GetDrawData() to obtain it and run your rendering function (up to v1.60, this used to call io.RenderDrawListsFn(). Nowadays, we allow and prefer calling your renderMesh function yourself.)
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after Render() and until the next call to NewFrame(). this is what you have to renderMesh.
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor type, reset in ImGui::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will renderMesh those for you
...
struct ImGuiStyle
{
    ...
    bool        AntiAliasedLinesUseTex;     // Enable anti-aliased lines/borders using textures where possible. Require back-end to renderMesh with bilinear filtering. Latched at the beginning of the frame (copied to ImDrawList).
...
//  A) Change your GPU renderMesh state,
//  B) renderMesh a complex 3D scene inside a UI element without an intermediate texture/render target, etc.
...
// Special Draw callback value to request renderer back-end to reset the graphics/renderMesh state.
// The renderer back-end needs to handle this special value, otherwise it will crash trying to call a function at this address.
// This is useful for example if you submitted callbacks which you know have altered the renderMesh state and you want it to be restored.
// It is not done by default because they are many perfectly useful way of altering renderMesh state for imgui contents (e.g. changing shader/blending settings before an Image call).
#define ImDrawCallback_ResetRenderState     (ImDrawCallback)(-1)

// Typically, 1 command = 1 GPU draw call (unless command is a callback)
// - VtxOffset/IdxOffset: When 'io.BackendFlags & ImGuiBackendFlags_RendererHasVtxOffset' is enabled,
//   those fields allow us to renderMesh meshes larger than 64K vertices while keeping 16-bit indices.
...
enum ImDrawListFlags_
{
    ...
    ImDrawListFlags_AntiAliasedLinesUseTex  = 1 << 1,  // Enable anti-aliased lines/borders using textures when possible. Require back-end to renderMesh with bilinear filtering.
...
struct ImDrawList
{
    // This is what you have to renderMesh
    ...
    IMGUI_API void  PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect = false);  // Render-level scissoring. This is passed down to your renderMesh function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
    ...
    // Advanced: Channels
    // - Use to split render into layers. By switching channels to can renderMesh out-of-order (e.g. submit FG primitives before BG primitives)
    ...
    // - We renderMesh triangles (three vertices)
...
// All draw data to renderMesh a Dear ImGui frame
// (NB: the style and the naming convention here is a little inconsistent, we currently preserve them for backward compatibility purpose,
// as this is one of the oldest structure exposed by the library! Basically, ImDrawList == CmdList)
struct ImDrawData
{
    bool            Valid;                  // Only valid after Render() is called and before the next NewFrame() is called.
    ImDrawList**    CmdLists;               // Array of ImDrawList* to renderMesh. The ImDrawList are owned by ImGuiContext and only pointed to from here.
    int             CmdListsCount;          // Number of ImDrawList* to renderMesh
    int             TotalIdxCount;          // For convenience, sum of all ImDrawList's IdxBuffer.Size
    int             TotalVtxCount;          // For convenience, sum of all ImDrawList's VtxBuffer.Size
    ImVec2          DisplayPos;             // Upper-left position of the viewport to renderMesh (== upper-left of the orthogonal projection matrix to use)
    ImVec2          DisplaySize;            // Size of the viewport to renderMesh (== io.DisplaySize for the main viewport) (DisplayPos + DisplaySize == lower-right of the orthogonal projection matrix to use)
    ...
    IMGUI_API void  DeIndexAllBuffers();                    // Helper to convert all buffers from indexed to non-indexed, in case you cannot renderMesh indexed. Note: this is slow and most likely a waste of resources. Always prefer indexed rendering!
...
//  - One or more fonts.
//  - Custom graphics data needed to renderMesh the shapes needed by Dear ImGui.
...
struct ImFontAtlas
{
    ...
    // After calling Build(), you can query the rectangle position and renderMesh your pixels.
    // You can also request your rectangles to be mapped as font glyph (given a font + Unicode point),
    // so you can renderMesh e.g. custom colorful icons and use them as regular glyphs.
...
struct ImFont
{
    ...
    // Members: Hot ~28/40 bytes (for CalcTextSize + renderMesh loop)
